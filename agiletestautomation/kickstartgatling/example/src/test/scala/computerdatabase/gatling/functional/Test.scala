...
import computerdatabase.gatling.{ComputerDatabaseChainBuilder, ComputerDatabaseHttpProtocolBuilder}
...
class Test extends ConfigurableSimulation {
  ...
  private val httpProtocol = ComputerDatabaseHttpProtocolBuilder.create()
    .baseURL(getBaseURL)
    .build
